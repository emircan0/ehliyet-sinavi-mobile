    ...
    return (
        <ScreenLayout className="bg-[#F8FAFC]">
            ...
            <View className="px-6 pt-4 pb-4 bg-white border-b border-slate-100">
                ...
                <Text className="text-slate-500 text-sm font-medium">Gerçek zamanlı performans analizin.</Text>
            </View>
            ...
            >
                {performanceData.totalQuestions === 0 ? (
                    ...
                    <View className="px-6 mt-16 items-center justify-center">
                        ...
                        <Text className="text-xl font-bold text-slate-900 mb-2">Henüz Veri Yok</Text>
                        ...
                        >
                            <Text className="text-blue-700 font-bold">Soru Çözmeye Başla</Text>
                        </TouchableOpacity>
                    </View>
                ) : (
                    ...
                    <>
                        <View className="px-6 mt-6 flex-row flex-wrap justify-between gap-y-4">
                            <StatCard icon={Target} label="Toplam Soru" value={performanceData.totalQuestions.toString()} color="#2563eb" bgColor="bg-blue-50" />
                            <StatCard icon={TrendingUp} label="Genel Başarı" value={`%${performanceData.successRate}`} color="#059669" bgColor="bg-emerald-50" />
                            <StatCard icon={Clock} label="Çözülen Sınav" value={performanceData.totalExams.toString()} color="#d97706" bgColor="bg-amber-50" />
                            <StatCard icon={Zap} label="Doğru Sayısı" value={performanceData.correctAnswers.toString()} color="#9333ea" bgColor="bg-purple-50" />
                        </View>

                        <View className="px-6 mt-8">
                            <Text className="text-lg font-bold text-slate-900 mb-4">Konu Performansı</Text>

                            <View className="relative">
                                {/* İçerik */}
                                <View className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm gap-6 overflow-hidden">
                                    {performanceData.categories.map((cat) => (
                                        <View key={cat.id}>
                                            <View className="flex-row justify-between mb-2">
                                                <Text className="text-slate-900 font-bold text-xs">{cat.name}</Text>
                                                <Text className="font-bold text-xs text-slate-500">%{cat.rate}</Text>
                                            </View>
                                ...
                                {!isPro && (
                                    <>
                                        <BlurView intensity={20} tint="light" className="absolute inset-0 z-10 rounded-3xl overflow-hidden" />
                                        <View className="absolute inset-0 z-20 items-center justify-center p-4">
                                            <View className="bg-white/90 px-6 py-4 rounded-3xl items-center shadow-lg border border-slate-100 w-full">
                                                <View className="bg-amber-100 w-12 h-12 rounded-full items-center justify-center mb-2">
                                                    <AlertCircle size={24} color="#d97706" />
                                                </View>
                                                <Text className="text-slate-900 font-bold text-base mb-1">Pro Analiz</Text>
                        ...
                        <View className="px-6 mt-8">
                            <Text className="text-lg font-bold text-slate-900 mb-4">Yapay Zeka Analizi</Text>

                            <View className="relative">
                                {/* İçerik */}
                                <View className="overflow-hidden rounded-2xl">
                                    {performanceData.successRate < 70 ? (
                                        <View className="bg-red-50 p-4 rounded-2xl border border-red-100 flex-row items-start">
                                            <AlertCircle size={20} color="#ef4444" className="mr-3 mt-0.5" />
                                            <View className="flex-1">
                                                <Text className="text-red-900 font-bold text-sm mb-1">Gelişmen Gerekiyor</Text>
                                                <Text className="text-red-800/80 text-xs leading-5">
                                                    Başarı oranın barajın altında. Özellikle düşük performans gösterdiğin konulara tekrar göz atmalısın.
                                                </Text>
                                            </View>
                                        </View>
                                    ) : (
                                        <View className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100 flex-row items-start">
                                            <CheckCircle2 size={20} color="#10b981" className="mr-3 mt-0.5" />
                                            <View className="flex-1">
                                                <Text className="text-emerald-900 font-bold text-sm mb-1">Harika Gidiyorsun!</Text>
                                                <Text className="text-emerald-800/80 text-xs leading-5">
                                ...
                                {!isPro && (
                                    <>
                                        ...
                                        <View className="absolute inset-0 z-20 items-center justify-center p-2">
                                            <View className="bg-white/90 px-6 py-4 rounded-3xl items-center shadow-lg border border-slate-100 w-full flex-row">
                                                <View className="bg-amber-100 w-10 h-10 rounded-full items-center justify-center mr-3">
                                                    <Zap size={20} color="#d97706" />
                                                </View>
                                                <View className="flex-1">
                                                    <Text className="text-slate-900 font-bold text-[13px] mb-0.5">Yapay Zeka Raporu</Text>
                                                    <TouchableOpacity onPress={() => {
                                                        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                                                        router.push('/premium');
                                                    }}>
                                                        <Text className="text-blue-600 font-bold text-xs">Premium Edin →</Text>
                                                    </TouchableOpacity>
                                                </View>
                                            </View>
                                        </View>
...
const StatCard = ({ icon: Icon, label, value, color, bgColor }: StatCardProps) => (
    <View className="w-[48%] bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
        <View className={`${bgColor} self-start p-2 rounded-lg mb-3`}>
...
const StatisticsSkeleton = () => (
    <ScreenLayout className="bg-[#F8FAFC]">
        <View className="px-6 pt-4 pb-4 bg-white border-b border-slate-100">
            <View className="h-8 w-48 bg-slate-200 rounded animate-pulse mb-2" />
            <View className="h-4 w-64 bg-slate-200 rounded animate-pulse" />
        </View>

        <View className="px-6 mt-6 flex-row flex-wrap justify-between gap-y-4">
            {[1, 2, 3, 4].map((i) => (
                <View key={i} className="w-[48%] bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                    <View className="h-8 w-8 bg-slate-200 rounded-lg mb-3 animate-pulse" />
                    <View className="h-8 w-16 bg-slate-200 rounded mb-1 animate-pulse" />
        ...
        <View className="px-6 mt-8">
            <View className="h-6 w-40 bg-slate-200 rounded mb-4 animate-pulse" />
            <View className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm gap-6">
                {[1, 2, 3, 4].map((i) => (
                    <View key={i}>
                        <View className="flex-row justify-between mb-2">
                            <View className="h-4 w-32 bg-slate-200 rounded animate-pulse" />
                            <View className="h-4 w-8 bg-slate-200 rounded animate-pulse" />
                        </View>
                        <View className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                            <View className="h-full bg-slate-200 w-1/2 animate-pulse rounded-full" />
                        </View>
                    </View>
                ))}
            </View>
        ...
        <View className="px-6 mt-8">
            <View className="h-6 w-40 bg-slate-200 rounded mb-4 animate-pulse" />
            <View className="h-[88px] w-full bg-slate-200 rounded-2xl animate-pulse" />
        </View>
    </ScreenLayout>
);