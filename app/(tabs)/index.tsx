...
// --- ÖRNEK BİLDİRİM VERİLERİ ---
const MOCK_NOTIFICATIONS = [
    { id: 1, title: 'Tebrikler, Harika Gidiyorsun! 🎉', message: 'Son çözdüğün Trafik Adabı testinde %90 başarı yakaladın.', time: '1 saat önce', icon: Award, color: '#10b981', bg: 'bg-emerald-50', isRead: false },
    { id: 2, title: 'Direksiyon Başına! 🚗', message: 'Bugün henüz hızlı antrenman yapmadın. Sadece 5 dakikanı alır.', time: '5 saat önce', icon: Clock, color: '#f59e0b', bg: 'bg-amber-50', isRead: false },
    { id: 3, title: 'Yapay Zeka Raporu Hazır', message: 'AI Hoca senin için zayıf olduğun konulardan bir test hazırladı.', time: '1 gün önce', icon: Sparkles, color: '#3b82f6', bg: 'bg-blue-50', isRead: true },
    { id: 4, title: 'Sistem Güncellemesi', message: 'Yeni trafik işaretleri soruları veritabanına eklendi.', time: '3 gün önce', icon: Info, color: '#8b5cf6', bg: 'bg-violet-50', isRead: true },
];
...
export default function Home() {
    ...
    return (
        <ScreenLayout className="bg-[#F8FAFC]">
            ...
            {/* --- HEADER --- */}
            <View className="px-6 py-2 flex-row justify-between items-center bg-[#F8FAFC] z-10 mt-2">
                <View className="flex-1 pr-4">
                    <View className="flex-row items-center mb-1">
                        <Sparkles size={14} color="#3b82f6" />
                        <Text className="text-blue-600 text-[10px] font-black tracking-widest uppercase ml-1">
                            Ehliyet Cepte
                        </Text>
                    </View>
                    <Text className="text-[28px] font-black text-slate-900 tracking-tight" numberOfLines={1}>
                ...
                <TouchableOpacity
                    ...
                    className="w-11 h-11 bg-white rounded-full items-center justify-center border border-slate-100 shadow-sm shadow-slate-200 active:opacity-70"
                >
            ...
            >
                ...
                <View className="mb-8">
                    <View className="px-6 flex-row justify-between items-center mb-4">
                        <Text className="text-[19px] font-black text-slate-900 tracking-tight">Hızlı Göz At</Text>
                    </View>
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 24, gap: 14 }}>
                        {studyTools.map((tool) => (
                            <TouchableOpacity
                                key={tool.id}
                                activeOpacity={0.7}
                                className={`w-[135px] p-4 rounded-[28px] bg-white border ${tool.border} justify-between h-[150px] shadow-sm shadow-slate-200/40 opacity-${isPro ? '100' : '75'}`}
                                onPress={() => handlePremiumFeature(`/study/${tool.id}`)}
                            >
                                <View className={`w-12 h-12 rounded-2xl ${tool.bg} items-center justify-center mb-2`}>
                                    <tool.icon size={22} color={tool.color} />
                                </View>
                                <View>
                                    <Text className="text-[14px] font-extrabold text-slate-900 leading-tight mb-1" numberOfLines={2}>{tool.title}</Text>
                                    <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-wider" numberOfLines={1}>{tool.subtitle}</Text>
                                    {!isPro && (
                                        <View className="absolute top-2 right-2 bg-slate-900/10 p-1.5 rounded-full">
                ...
                <View className="px-6">
                    <Text className="text-[19px] font-black text-slate-900 tracking-tight mb-4">Konu Testleri</Text>
                    <View className="flex-row flex-wrap justify-between gap-y-4">
                        {categories.map((cat) => (
                            <TouchableOpacity
                                key={cat.id}
                                activeOpacity={0.7}
                                className="w-[48%] bg-white p-5 rounded-[28px] border border-slate-100 shadow-sm shadow-slate-200/50 flex-col h-[150px]"
                                onPress={() => {
                                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                                    router.push({ pathname: '/quiz/[id]', params: { id: cat.id } });
                                }}
                            >
                                <View className={`w-12 h-12 rounded-2xl ${cat.bg} items-center justify-center mb-3`}>
                                    <cat.icon size={24} color={cat.color} />
                                </View>
                                <Text className="font-extrabold text-slate-900 text-[15px] leading-5" numberOfLines={2}>{cat.name}</Text>
                                <View className="flex-row items-center justify-between mt-auto pt-2 border-t border-slate-50">
                                    <Text className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Testi Çöz</Text>
            ...
            <Modal visible={showNotifications} animationType="slide" transparent>
                <View className="flex-1 justify-end bg-slate-900/40">
                    <View className="bg-[#F8FAFC] rounded-t-[32px] h-[75%] shadow-2xl overflow-hidden">

                        {/* Modal Header */}
                        <View className="px-6 py-5 bg-white border-b border-slate-100 flex-row justify-between items-center">
                            <View>
                                <Text className="text-xl font-black text-slate-900 tracking-tight">Bildirimler</Text>
                                {unreadCount > 0 && (
                                    <Text className="text-blue-600 text-xs font-bold mt-0.5">{unreadCount} yeni bildirim</Text>
                                )}
                            </View>
                            <View className="flex-row gap-3">
                                <TouchableOpacity className="bg-slate-50 px-3 py-2 rounded-xl border border-slate-100 items-center justify-center">
                        ...
                        <ScrollView className="flex-1 px-6 pt-4" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
                            {MOCK_NOTIFICATIONS.map((notif) => (
                                <TouchableOpacity
                                    key={notif.id}
                                    activeOpacity={0.7}
                                    className={`mb-3 p-4 rounded-[24px] border flex-row items-start ${notif.isRead ? 'bg-white border-slate-100' : 'bg-blue-50/50 border-blue-100'}`}
                                >
                                    <View className={`w-12 h-12 rounded-2xl items-center justify-center mr-4 ${notif.bg}`}>
                                        <notif.icon size={20} color={notif.color} />
                                    </View>
                                    <View className="flex-1">
                                        <View className="flex-row justify-between items-start mb-1">
                                            <Text className={`font-bold text-[15px] flex-1 mr-2 ${notif.isRead ? 'text-slate-800' : 'text-slate-900'}`}>
                                                {notif.title}
                                            </Text>
                                            {!notif.isRead && <View className="w-2 h-2 bg-blue-600 rounded-full mt-1.5" />}
                                        </View>
                                        <Text className="text-slate-500 text-[13px] leading-5 mb-2">{notif.message}</Text>
                                        <Text className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">{notif.time}</Text>
                                    </View>
                                </TouchableOpacity>
                            ))}
                        </ScrollView>
